...
const SHATTER_PIECES = Array.from({ length: SHATTER_GRID * SHATTER_GRID }).map((_, i) => {
  ...
  // Menghitung arah ledakan menjauhi titik tengah (eksplosif)
  const dx = (col - centerX) * (Math.random() * 120 + 80);
  const dy = (row - centerY) * (Math.random() * 120 + 80);
  // Rotasi acak hingga 720 derajat
...
export default function Home() {
  ...
  // Efek memantau posisi scroll yang sangat mulus
  useEffect(() => {
    let ticking = false;
    const handleScroll = () => {
      if (!ticking) {
  ...
  // FASE 1: Progress ledakan logo (dari scroll 0 hingga 400px)
  const shatterProgress = Math.min(Math.max(scrollY / 400, 0), 1);
  
  // FASE 2: Progress muncul teks dari bawah (dimulai SETELAH logo pecah, yaitu di scroll 450px hingga 850px)
  const textRevealProgress = Math.min(Math.max((scrollY - 450) / 400, 0), 1);
  ...
  return (
    <div className="bg-[#030303] overflow-x-hidden relative transition-colors duration-1000">
      ...
      {/* 1. EPIC HERO SECTION (Tinggi ditambah ke 250vh agar ada ruang scroll santai) */}
      <section className="h-[250vh] relative z-10 bg-[#030303]">
        <div className="sticky top-0 h-screen w-full flex flex-col items-center justify-center overflow-hidden px-6">
          
          {/* Glow Merah Latar Belakang */}
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[300px] md:w-[600px] h-[300px] md:h-[600px] bg-red-600/10 rounded-full blur-[150px] pointer-events-none"></div>

          {/* FASE 1: EFEK SHATTER LOGO SVG (64 Serpihan) */}
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-64 h-64 md:w-96 md:h-96 lg:w-[450px] lg:h-[450px] pointer-events-none z-20">
            {SHATTER_PIECES.map((piece, i) => (
              <div
                ...
                style={{
                  ...
                  /* Pastikan SVG lu ada di path ini */
                  ...
                  // Pecahan memudar lebih cepat saat mendekati akhir animasi
                  ...
                  `,
                  willChange: 'transform, opacity' // Optimasi performa agar smooth
                }}
              />
            ))}
          </div>

          {/* FASE 2: TEKS MUNCUL DARI BAWAH SETELAH LOGO HILANG */}
          <div
            className="absolute inset-0 flex flex-col items-center justify-center text-center pointer-events-none z-10"
            style={{
              opacity: textRevealProgress, // Opacity murni dari 0 ke 1
              // Teks diletakkan jauh di bawah (150px) dan naik seiring scroll
              transform: `translateY(${150 * (1 - textRevealProgress)}px) scale(${0.95 + 0.05 * textRevealProgress})`,
              filter: `blur(${(1 - textRevealProgress) * 15}px)`
            }}
          >
            <div className="flex items-center justify-center gap-4 mb-6">
              <div className="w-12 h-px bg-red-600"></div>
              <span className="text-[9px] md:text-[10px] font-black tracking-[0.5em] text-red-500 uppercase">Creative Digital Agency</span>
              <div className="w-12 h-px bg-red-600"></div>
            </div>
            <h1 className="text-5xl sm:text-7xl md:text-8xl lg:text-[10rem] font-black text-white leading-[0.85] tracking-tighter uppercase mb-6 mix-blend-difference drop-shadow-2xl">
          ...
          {/* Indikator Panah Bawah - Hilang saat logo mulai pecah */}
          <div className="absolute bottom-10 flex items-center justify-center w-full pointer-events-none" style={{ opacity: 1 - (scrollY / 200) }}>
      ...
      <div className="relative z-10 pt-20">
        {/* 2. Visual Story Grid (Bento Grid Fix agar Gambar Tidak Mendelep) */}
        ...
        <section className="py-24 md:py-32 px-6 md:px-12 lg:px-24 max-w-[100rem] mx-auto border-t border-white/5 relative z-10">
          <Reveal>
