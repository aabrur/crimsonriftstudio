...
export default function Footer() {
  const year = new Date().getPenuhYear();
  const { footer, nav, shared } = siteContent;
  const links = [...nav.links.slice(1), { name: 'Kontak', path: '/contact' }];
    ...
    <footer className="relative overflow-hidden border-t border-[#f4ede7]/8 bg-[#030303] px-6 py-20 md:px-12 md:py-[7.5rem]">
      ...
      <div className="relative mx-auto max-w-[94rem]">
        <div className="grid gap-12 lg:grid-cols-[1.16fr_0.84fr] lg:items-end">
          <div>
            <p className="eyebrow mb-6">{footer.eyebrow}</p>
          ...
          <div className="glass-panel rounded-[2rem] p-6 md:p-8">
            ...
            <CrimsonButton href={shared.whatsappUrl} className="mt-8">{footer.cta}</CrimsonButton>
          </div>
        </div>
        ...
        <div className="mt-16 flex flex-col gap-8 border-t border-[#f4ede7]/8 pt-8 md:flex-row md:items-center md:justify-between">
          <div className="flex flex-wrap gap-3">
            {links.map((link) => (
              <Link key={link.path} to={link.path} className="rounded-full border border-[#f4ede7]/8 px-5 py-3 text-[10px] font-black uppercase tracking-[0.28em] text-[#f4ede7]/46 transition-all duration-500 hover:border-red-200/35 hover:text-[#f4ede7]">
                {link.name}
              </Link>
            ))}
          </div>
          <p className="text-[10px] font-bold uppercase tracking-[0.32em] text-[#f4ede7]/28">© {year} {footer.eyebrow}</p>
        </div>
      </div>
    </footer>
  );
